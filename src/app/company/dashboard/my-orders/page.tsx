                            ...
                            <div className="border-b border-black py-2 text-center bg-slate-50/50">
                                {order.payment_method === "cod" ? (
                                    <div className="border border-black p-2 bg-slate-100 rounded">
                                        ...
                                        <p className="text-[6px] text-slate-600 font-semibold mt-0.5">(Includes ₹{order.cod_charges ?? 100} COD Handling Charge)</p>
                                    </div>
                                ) : (
                                    <div className="border border-black p-2 bg-slate-100 rounded">
        ...
        <div className="min-h-screen bg-[#F5F6F8] font-sans text-slate-900 pb-20 selection:bg-indigo-100">
            ...
            <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-slate-200">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
                    <div>
                        <h1 className="text-sm font-semibold tracking-tight text-slate-900 leading-none">Order Management</h1>
                        <p className="text-[11px] text-slate-500 font-medium hidden sm:block mt-0.5">Track and fulfill customer orders</p>
                    </div>
                    <div className="flex items-center gap-2 bg-indigo-50 border border-indigo-100 px-3 py-1 rounded-full">
                        ...
                        <span className="text-[11px] font-semibold text-indigo-700">Live Sync</span>
                    </div>
                </div>
            </header>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-8">
                    ...
                    <Card className="rounded-2xl border-slate-200 shadow-xs bg-white">
                        <CardContent className="p-4 sm:p-5 flex flex-col justify-between h-full">
                            <div className="flex justify-between items-start mb-2">
                                <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Active (Pending)</p>
                    ...
                    <Card className="rounded-2xl border-slate-200 shadow-xs bg-white">
                        <CardContent className="p-4 sm:p-5 flex flex-col justify-between h-full">
                            <div className="flex justify-between items-start mb-2">
                                <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">Revenue (Gross)</p>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center mb-4 gap-4">
                    <div className="relative w-full sm:w-96">
                        ...
                        <Input
                            ...
                            className="pl-9 h-10 rounded-xl bg-white border-slate-200 text-xs focus:border-indigo-500 focus:ring-0 shadow-xs w-full font-medium"
                        />
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    {filteredOrders.length === 0 ? (
                        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-slate-200">
                            ...
                            <p className="text-xs text-slate-500 mt-1 max-w-sm mx-auto">We couldn't find any orders matching your search criteria.</p>
                        </div>
                    ) : (
                        filteredOrders.map((order) => (
                            <Card key={order.id} className="rounded-2xl border-slate-200 shadow-xs overflow-hidden bg-white">
                                
                                {/* Order Header */}
                                <div className="bg-slate-50/50 border-b border-slate-100 px-4 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-3 mb-1">
                                            <span className="text-sm font-semibold text-slate-900 tracking-tight">
                                                Order #{order.id.substring(0, 8).toUpperCase()}
                                            </span>
                                            <Badge variant="outline" className={`px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider rounded-full border ${getStatusBadgeClass(order.status)}`}>
                                                {order.status}
                                            </Badge>
                                        </div>
                                    ...
                                    <div className="flex items-center gap-2 w-full sm:w-auto">
                                        <Button 
                                            ...
                                            className="h-8 px-3 text-xs font-semibold text-slate-700 border-slate-200 hover:bg-slate-50 rounded-xl flex items-center gap-1.5"
                                        >
                                        ...
                                        {order.status !== "delivered" && order.status !== "cancelled" && (
                                            <Button 
                                                ...
                                                className="h-8 px-3.5 text-xs font-semibold bg-indigo-600 text-white hover:bg-indigo-700 rounded-xl shadow-xs"
                                            >
