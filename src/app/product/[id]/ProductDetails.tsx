...
const getPublicUrlFromPath = (path: string | undefined): string => {
  ...
  if (path.startsWith('http')) {
    // Check for double-prepended Supabase URLs
...
export default function ProductDetails({ product }: ProductDetailsProps) {
  ...
  return (
    <div className="bg-white pb-28 sm:pb-0 relative">
      {/* Breadcrumb */}
      <nav className="hidden sm:flex items-center text-sm text-gray-500 px-4 sm:px-6 lg:px-8 py-4">
        <Link href="/" className="hover:text-gray-900 transition-colors">Home</Link>
        <ChevronRight className="w-4 h-4 mx-2 text-gray-400" />
        <Link href="/shop" className="hover:text-gray-900 transition-colors">Products</Link>
        <ChevronRight className="w-4 h-4 mx-2 text-gray-400" />
      ...
      <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div className="lg:grid lg:grid-cols-2 lg:gap-x-12 xl:gap-x-16 ">
          ...
          {/* Product Images - Mobile Edge-to-Edge */}
          <div className="lg:max-w-lg lg:self-start">
            {/* Main Image View */}
            <div className="relative w-full aspect-square sm:rounded-3xl overflow-hidden bg-gray-50 mb-4 border-b border-gray-100 sm:border-0">
              <AnimatePresence mode="wait">
                ...
                >
                  <img src={selectedImage || "/placeholder.svg"} alt={product.productName} className="object-cover w-full h-full absolute inset-0" />
                </motion.div>
              </AnimatePresence>
            </div>

            {/* Gallery Thumbnails */}
            <div className="flex gap-3 px-4 sm:px-0 overflow-x-auto pb-4 custom-scrollbar">
              {resolvedImages.map((imgUrl, i) => (
                <button
                  key={i}
                  onClick={() => setSelectedImage(imgUrl)}
                  className={`relative w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 rounded-2xl overflow-hidden transition-all border-2 ${selectedImage === imgUrl ? "border-emerald-600" : "border-transparent hover:border-gray-300 opacity-80 hover:opacity-100"
                    }`}
                >
                  <img src={imgUrl || "/placeholder.svg"} alt={`Product image ${i + 1}`} className="object-cover w-full h-full absolute inset-0" />
                </button>
              ))}
              {product.productVideoUrl && (
                <button
                  onClick={() => window.open(getPublicUrlFromPath(product.productVideoUrl), "_blank")}
                  className="relative w-20 h-20 sm:w-24 sm:h-24 flex-shrink-0 rounded-2xl overflow-hidden bg-gray-100 flex items-center justify-center text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors border-2 border-transparent"
                >
          ...
          <div className="mt-10 lg:mt-0 lg:col-start-2 lg:row-span-2 lg:self-start">
            <div className="flex items-center gap-4 mb-6">
              <div className="relative h-12 w-12 overflow-hidden rounded-full border border-gray-200">
                <img src={getCompanyLogoUrlFromPath(product.company.logo) || "/placeholder.svg"} alt={product.company.name} className="object-cover w-full h-full absolute inset-0" />
              </div>
              <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-black sm:text-4xl">{product.productName}</h1>
            ...
            <div className="mt-8 border-t border-gray-200 pt-8">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <button onClick={handleDirectBuy} className="flex items-center justify-center gap-2 bg-white text-emerald-600 py-3 px-4 rounded-lg border-2 border-emerald-600 hover:bg-gray-100 transition-colors disabled:opacity-50 disabled:cursor-not-allowed" disabled={isActionDisabled}>
