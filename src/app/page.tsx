...
function CategoryCarousel({ categories, selectedCategory, onCategoryClick }: CategoryProps) {
  return (
    <section className="py-2 bg-slate-50/60 border-b border-slate-100">
          ...
          <div className="flex gap-2 sm:gap-2.5 w-max">
            {categories.map((cat) => (
              <button
                ...
                className={`flex-shrink-0 transition-all duration-200 ${selectedCategory === cat.title
                  ? "scale-[1.02]"
                  : "hover:scale-[1.01]"
                  }`}
              >
                <div
                  className={`flex items-center gap-2 px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-lg border transition-all whitespace-nowrap ${selectedCategory === cat.title
                ...
                >
                  ...
                  <div className="text-left">
                    <p className={`font-semibold text-xs sm:text-sm leading-tight ${selectedCategory === cat.title ? "text-emerald-800" : "text-slate-800"}`}>
                      {cat.title.split(" ")[0]}
                    </p>
                  </div>
                </div>
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen bg-white font-sans antialiased text-slate-900 selection:bg-emerald-100">
      ...
      <section className="py-2.5 sm:py-4 bg-[#fafafa]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between border-b border-slate-200/80 pb-2 mb-3">
            <h2 className="text-base sm:text-lg font-bold text-slate-900 tracking-tight flex items-center gap-2">
              <span className="w-1.5 h-4.5 bg-emerald-600 rounded-full inline-block"></span>
              Fresh <span className="text-emerald-600 font-extrabold">Picks</span>
            </h2>
            <Link href="/shop" className="text-xs font-semibold text-emerald-600 hover:text-emerald-700 flex items-center gap-1 transition-colors">
